...
* git reset --soft HEAD^ (undoes the last commit, so that committed files are added back to the "staging area")
* git reset --hard HEAD^ (undoes the last commit without adding committed files back to the "staging area", so all changes are gone and git status shows nothing)
* git reset --hard HEAD^^ (undoes the last two commits without adding committed files back to the "staging area")
